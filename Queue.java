class Queue {
	private char[] q;
	private int putloc, getloc; //указатели на индексы для помещения и извлечения симоволов
	//Создаётся пустая очередь заданного размера
	Queue(int size) {
		q = new char[size]; //выделяем память под очередь нужного размера
		putloc=getloc=0;
	}
	//Создаётся очередь на основе другого объекта очереди
	//
	Queue(Queue ob) {
		putloc = ob.putloc;
		getloc = ob.getloc;
	...
	//Создание очереди с начальным значениями на основе массива
	Queue(char[] a) {
		putloc = 0;
		getloc = 0;
		q = new char[a.length];
